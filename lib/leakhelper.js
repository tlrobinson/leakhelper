(function(LeakHelper) {
    ...
    Context.prototype.traverse = function traverse(object, path) {
        ...
        while (queue.length > 0 || secondQueue.length > 0) {
            ...
            if ((type === "object" && object) || type === "function") {
                // other well-known properties that aren't usually enumerable
                // NOTE: we need to use an array because it will contain "__proto__"
                var properties = LeakHelper.nonEnumerables.slice(); // copy

                // enumerate object's properties
                for (var property in object) {
                    if (LeakHelper.nonEnumerables.indexOf(property) < 0)
                        properties.push(property);
                }

                properties.forEach(function(property) {
                    var newItem = { object: object[property], path : path.concat(property) };
                    if (Object.prototype.hasOwnProperty.call(object, property)) {
                        queue.push(newItem);
                    } else if (this.traversePrototypes) {
                        secondQueue.push(newItem);
                    }
                }, this);
            }
        }
